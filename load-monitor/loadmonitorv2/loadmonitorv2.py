...
class Loadmonitorv2(object):
    ...
    def launch_loadtest(self, loadtest_params):
        ...
        if not self.is_test_running(self._name_from_id(loadtest_params['server'])):
            cmd = 'cd {loadtest_path} && {loadtest_path}/load-tester -c {loadtest_path}/etc/conf-{servername}.py -d {lmv2_path}/logs/sip_logs/{servername}/ {loadtest_path}/scenarios/call-then-hangup/'.format(loadtest_path = self.xivo_loadtest, servername = self._name_from_id(loadtest_params['server']), lmv2_path = '/var/www/load-monitor-v2')
            print('################ COMMAND:')
            print(cmd)
            try:
